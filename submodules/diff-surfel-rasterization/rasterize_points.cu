	...
	const bool debug)
{
  ...
  */
  if (means3D.ndimension() != 2 || means3D.size(1) != 3) {
   ...
   //std::cout<<"opa4451"<<opacity[4451]<<std::endl;
  ...
  torch::Tensor trianBuffer = torch::empty({0}, options.device(device));
  torch::Tensor binningBuffer = torch::empty({0}, options.device(device));
  torch::Tensor imgBuffer = torch::empty({0}, options.device(device));
  std::function<char*(size_t)> geomFunc = resizeFunctional(geomBuffer);
  std::function<char*(size_t)> trianFunc = resizeFunctional(trianBuffer);
  ...
  /*
  clock_t end = clock();
  std::cout<<"rasp_t"<<end-start0<<std::endl;
  
  std::cout<<"clk"<<CLOCKS_PER_SEC<<std::endl;
  cudaEventRecord(stop);
    cudaEventSynchronize(stop);
    float elapse_time;
    cudaEventElapsedTime(&elapse_time, start, stop);
    printf("point_elapse_time %f\n", elapse_time);
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_drotations = torch::zeros({P, 4}, means3D.options());
   ...
   //std::cout<<"dL_dtransMat"<<dL_dtransMat[9734,0]<<std::endl;
